...
# 加载数据
    ...
    def process_all_queries(self):
        ...
        for idx, row in self.search_results.iterrows():
            # 显示进度 - 每500个查询显示一次，并显示这500次查询所用时间
            if idx % 500 == 0 and idx > 0:
                batch_end_time = time.time()
                batch_time = batch_end_time - batch_start_time
                progress = idx / len(self.search_results) * 100
                print(f"  进度: {idx}/{len(self.search_results)} ({progress:.1f}%) | 500次查询耗时: {batch_time:.2f}秒")
                batch_start_time = batch_end_time
            
            # 最后一条记录也显示进度
            if idx == len(self.search_results) - 1:
                batch_end_time = time.time()
                batch_time = batch_end_time - batch_start_time
                progress = 100.0
                print(f"  进度: {idx+1}/{len(self.search_results)} ({progress:.1f}%) | {idx % 500 + 1}次查询耗时: {batch_time:.2f}秒")
