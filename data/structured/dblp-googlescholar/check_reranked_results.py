...
try:
    results = pd.read_parquet('dblp-googlescholar_reranked_results_k50.parquet')
    ...
    print("\n=== Checking reranked results consistency ===")
    ...
    print("Reranked results file not found")

# 检查评估报告
print("\n=== Evaluation Report ===")
try:
    with open('dblp-googlescholar_evaluation_report.json', 'r') as f:
        import json
        report = json.load(f)
        print("Report keys:", list(report.keys()))
        if 'evaluation_available' in report and report['evaluation_available']:
            print("Evaluation available")
            for k in [1, 5, 10]:
                if f'recall_improvement@{k}' in report:
                    print(f"Recall@{k} improvement: {report[f'recall_improvement@{k}']}")
        else:
